...
Una vez convencidos de que $\sqrt{2}$ es efectivamente un número *real* procedemos a probar que no es racional con un argumento por el absurdo. Intentar replicar esta idea para, por ejemplo, $\sqrt[3]{2}$ nos lleva a la famosa imposibilidad de *doblar el cubo* y en general, para la raíz de orden arbitrario, a las restricciones que impone la teoría de Galois a las extensiones de cuerpos.   
