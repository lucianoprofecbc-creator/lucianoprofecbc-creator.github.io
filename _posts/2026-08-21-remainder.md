...
$$\left(\frac{f^{k+1}(t)(x-t)^{k+1}}{(k+1)!}\right)'=\frac{f^{k+2}(t)(x-t)^{k+1}}{(k+1)!}-\frac{f^{k+1}(t)(x-t)^{k}}{k!}$$

y entonces es una suerte de *telescópica*, cuando calculamos $\phi'(t)$ se nos cancelan todos los términos al sumar, excepto el último: 
